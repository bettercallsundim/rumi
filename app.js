 ...
 function onepagescroll(selector, options) {
	...
	if(document.readyState === 'complete')
      ...
      const $$=document.querySelectorAll.bind(document);
        let allnavbutton=$$('.ops-navigation li a');
        let sective=$$('.main section');
      ...
      $$('.actual li a').forEach((element,ind)=> {
         element.onclick=()=>{
            ...
            allnavbutton.forEach((item)=>{
               item.classList.remove('active');

            })
            sective.forEach((sec,ind2)=>{
               if(sec.querySelector(myid)){
                  allnavbutton[ind2].classList.add('active');
                  changePage(null,pages.length,ind2);
               }
            })
         }
       });
